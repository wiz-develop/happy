...
class WCP_Posts_Carousel
{
	...
	function admin_options_page_scripts($slug){
		...
		if ($slug == 'wcp_carousel_page_posts_carousel') {
			...
			wp_localize_script( 'photo-book-admin-js', 'wcpAjax', array(
				'url' => admin_url( 'admin-ajax.php' ),
				'path' => plugin_dir_url( __FILE__ ),
				'nonce' => wp_create_nonce( 'wcp_posts_carousel_admin' ),
			));
		}
	}
	...
	function save_carousels(){
		...
		check_ajax_referer( 'wcp_posts_carousel_admin', 'nonce' );
		$carousels = isset( $_POST['carousels'] ) ? $this->sanitize_carousel_data( wp_unslash( $_POST['carousels'] ) ) : array();
		update_option( 'wcp_posts_carousel', array( 'carousels' => $carousels ) );
