...
class WCP_Responsive_Posts_Carousel
{
	...
	function render_shortcode($attrs){
		if (isset($attrs['id']) && $attrs['id'] != '') {
			...
			wp_enqueue_script( 'slick-js', RPC_URL.'/assets/slick.min.js', array('jquery') );
				$carousel_settings = get_post_meta( absint( $attrs['id'] ), 'carousel_meta', true );
				if ( ! is_array( $carousel_settings ) ) {
					return '';
				}

				if ( isset( $carousel_settings['hover_effect'] ) && $carousel_settings['hover_effect'] == '3d hover box') {
        ...
        if ( $slug == 'post-new.php' || $slug == 'post.php') {
            if (isset($post->post_type) && 'wcp_carousel' === $post->post_type) {
				...
				wp_enqueue_script( 'select2-js', RPC_URL.'/js/select2.min.js', array('jquery') );
					...
					wp_localize_script( 'carousel-admin', 'rpcAdmin', array(
						'nonce' => wp_create_nonce( 'rpc_carousel_admin' ),
					));
            }
        }		
	}
	...
	function rpc_get_posts(){
			...
			check_ajax_referer( 'rpc_carousel_admin', 'nonce' );
			$post_type = isset( $_POST['post_type'] ) ? sanitize_key( wp_unslash( $_POST['post_type'] ) ) : 'post';
			$all_posts = get_posts( array('post_type' => $post_type, 'posts_per_page' => -1 ) );
			echo '<option value="all">'.esc_html__( 'All', 'responsive-posts-carousel' ).' '.esc_html( $post_type ).'s</option>';
			foreach ($all_posts as $key => $post_obj) {
	...
	function rpc_get_terms(){
			...
			echo __( 'Sorry! this Taxonomy has no Terms.', 'wcp-carousel' );
				...
				foreach ($terms as $key => $value) {
					echo '<option value="'.esc_attr( $value->term_id ).'">'.esc_html( $value->name ).'('.absint( $value->count ).')</option>';
			}
			echo '</select>';			
		}
		die(0);
	...
	function save_carousel($post_id){
        ...
        if (isset($_POST['carousel_data']) && $_POST['carousel_data'] != '') {
	            update_post_meta( $post_id, 'carousel_meta', $this->sanitize_carousel_data( wp_unslash( $_POST['carousel_data'] ) ) );
	        }		
		}

		function sanitize_carousel_data( $value ) {
			if ( is_array( $value ) ) {
				return array_map( array( $this, 'sanitize_carousel_data' ), $value );
			}
	        ...
	        switch ($key) {
	            ...
	            default:
		            	if ($length != '') {
		            		...
		            		echo wp_kses_post( wp_trim_words( $meta, $length ) );
		            	} else {
		            		echo wp_kses_post( get_post_meta( $post_id, $key, true ) );
	            	}	            
